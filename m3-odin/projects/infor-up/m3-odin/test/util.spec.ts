...
import { ArrayUtil, NumUtil, INumberFormatOptions } from '../index';
...
describe('Util ArrayUtil', () => {
    ...
    it('should sort array', () => {
        ...
        expect(ArrayUtil.sortByProperty(array, 'foo')).toStrictEqual([value4, value3, value2, value2, value1]);
        expect(array).toStrictEqual([value4, value3, value2, value2, value1]); // array isn't immutable
        expect(ArrayUtil.sortByProperty(array, 'foo', { ignoreCase: true })).toStrictEqual([value3, value2, value2, value4, value1]);
        expect(ArrayUtil.sortByProperty([value1, value2, value3], 'bar')).toStrictEqual([value1, value2, value3]);
    });
    ...
    it('should remove item from array', () => {
        ...
        expect(array).toStrictEqual([value1, value3, value2]);
    });

    it('should remove item from array by property', () => {
        const array = [value1, value2, value3, value3];

        expect(ArrayUtil.removeByProperty(array, 'foo', value3.foo)).toStrictEqual(value3);
        expect(ArrayUtil.removeByProperty(array, 'foo', 'bar')).toBeNull();
        expect(ArrayUtil.removeByProperty(array, 'bar', value3.foo)).toBeNull();
        expect(array).toStrictEqual([value1, value2, value3]);
    });

    it('should remove item from array by predicate', () => {
        const array = [value1, value2, value3, value1];
        const predicate = jest.fn<(item: { foo: string; }) => boolean>()
            .mockReturnValueOnce(true)
            .mockReturnValueOnce(false)
            .mockReturnValueOnce(true)
            .mockReturnValue(false);

        expect(ArrayUtil.removeByPredicate(array, predicate)).toStrictEqual(value1);
        expect(ArrayUtil.removeByPredicate(array, predicate)).toStrictEqual(value3);
        expect(ArrayUtil.removeByPredicate(array, predicate)).toBeNull();
        expect(array).toStrictEqual([value2, value1]);
        expect(predicate).toHaveBeenCalledTimes(5);
        expect(predicate).toHaveBeenNthCalledWith(1, value1);
        expect(predicate).toHaveBeenNthCalledWith(2, value2);
        expect(predicate).toHaveBeenNthCalledWith(3, value3);
    });
    ...
    it('should find index of item from array by predicate', () => {
        ...
        expect(ArrayUtil.indexByPredicate(array, predicate)).toBe(-1);
        expect(array).toStrictEqual([value1, value2, value3, value1]);
        expect(predicate).toHaveBeenCalledTimes(7);
        expect(predicate).toHaveBeenNthCalledWith(1, value1);
        expect(predicate).toHaveBeenNthCalledWith(2, value1);
        expect(predicate).toHaveBeenNthCalledWith(3, value2);
    });
    ...
    it('should find index of item from array by property', () => {
        ...
        expect(array).toStrictEqual([value1, value2, value3, value3]);
    });
    ...
    it('should filter item from array by predicate', () => {
        ...
        expect(ArrayUtil.filterByPredicate(array, predicate)).toStrictEqual([value1, value3]);
        expect(array).toStrictEqual([value1, value2, value3, value1]);
        expect(predicate).toHaveBeenCalledTimes(4);
        expect(predicate).toHaveBeenNthCalledWith(1, value1);
        expect(predicate).toHaveBeenNthCalledWith(2, value2);
        expect(predicate).toHaveBeenNthCalledWith(3, value3);
        expect(predicate).toHaveBeenNthCalledWith(4, value1);
    });
    ...
    it('should conatin item from array by property', () => {
        ...
        expect(ArrayUtil.containsByProperty(array, 'bar', value3.foo)).toBe(false);
        expect(array).toStrictEqual([value1, value2, value3, value3]);
    });
    ...
    it('should find all items from array by predicate', () => {
        ...
        expect(ArrayUtil.findAll(array, predicate)).toStrictEqual([value1, value3]);
        expect(ArrayUtil.findAll(array, predicate)).toStrictEqual([]);
        expect(ArrayUtil.findAll(undefined as unknown as [], predicate)).toBeNull();
        expect(array).toStrictEqual([value1, value2, value3, value1]);
    ...
    it('should move item in array', () => {
        ...
        expect(array).toStrictEqual([value1, value3, value2, value1]);
        ArrayUtil.move(array, 1, 5);
        expect(array).toStrictEqual([value1, value2, value1, undefined, undefined, value3]);
    });
    ...
    it('should swap items in array', () => {
        ...
        expect(array).toStrictEqual([value1, undefined, value2, value1, , value3]);
    });
});
...
describe('Util NumUtil', () => {
    ...
    it('should return default options', () => {
        expect(NumUtil.getDefaultOptions()).toStrictEqual(NumUtil['defaultOptions']);
    });

    it('should modify default options', () => {
        const option = { foo: 'bar' } as INumberFormatOptions;
        NumUtil.setDefaultOptions(option);
        expect(NumUtil.getDefaultOptions()).toStrictEqual(option);
    });
    ...
    it('should return if there are integers', () => {
        ...
        expect(NumUtil.hasOnlyIntegers(undefined as unknown as string)).toBe(false);
...
describe('Util CoreUtil', () => {
    ...
    it('should return UUID', () => {
        Math.random = jest.fn<() => number>().mockReturnValue(0.5);
...
describe('Util StringUtil', () => {
    it('should check if starts with', () => {
        ...
        expect(StringUtil.startsWith(undefined as unknown as string, 'oo')).toBe(false);
    });

    it('should check if ends with', () => {
        expect(StringUtil.endsWith('foo', 'fo')).toBe(false);
        expect(StringUtil.endsWith('foo', 'oo')).toBe(true);
        expect(StringUtil.endsWith(undefined as unknown as string, 'oo')).toBe(false);
    });
...
describe('Util HttpUtil', () => {
    it('should return query', () => {
        expect(HttpUtil.getQuery()).toStrictEqual({});
    });
    ...
    it('should parse query', () => {
        expect(HttpUtil.parseQuery('?foo=bar&bar=foo')).toStrictEqual({ foo: 'bar', bar: 'foo' });
    });

    it('should create query', () => {
        expect(HttpUtil.toQuery({ foo: 'bar', bar: 'foo' })).toBe('foo=bar&bar=foo');
    });
});
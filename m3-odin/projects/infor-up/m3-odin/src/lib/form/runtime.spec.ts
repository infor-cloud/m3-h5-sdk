...
describe("Translator", () => {
   ...
   it("should be improved, because items is optional but throws error", () => {
      ...
      }).toThrowError("Cannot read properties of undefined (reading 'length')");
   });
   ...
   it("should be improved, because item.file is never undefined", () => {
      ...
      const requestWithFile: ITranslationRequest = {
         items: [{ key: "bar", file: "hello" }],
      };
   ...
   it("should parse response", () => {
      const jobBefore: ITranslationJob = {
         items: [{ key: "FA31001", file: "MVXCON" }],
         language: "GB",
      };
      const jobAfter: ITranslationJob = {
         items: [{ key: "FA31001", file: "MVXCON" }],
         language: "GB",
      };
      const response =
         '<?xml version="1.0" encoding="UTF-8" ?><Root mcv="1.0"><Result>0</Result><Texts language="GB"><Text file="MVXCON" key="FA31001">Tax Asset Group. Update</Text></Texts></Root>';

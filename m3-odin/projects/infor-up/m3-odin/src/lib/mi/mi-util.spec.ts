...
describe("MI Util", () => {
   ...
   it("should create update record", () => {
      const originalRecord = new MIRecord({
         CONO: 100,
         DIVI: 200,
         CUNO: "foo",
         SUNO: "bar",
      ...
      const newRecord = {
         DIVI: 300,
         CUNO: "bar",
         SUNO: originalRecord["SUNO"],
      };
      const updatedFields = ["CUNO", "SUNO"];
      const mandatoryFields = ["CONO", "DIVI"];
      const expectedRecord = new MIRecord({
         CONO: originalRecord["CONO"].toString(),
         DIVI: newRecord["DIVI"].toString(),
         CUNO: newRecord.CUNO,
         metadata: null,
      });
      expect(
         MIUtil.createUpdateRecord(
            originalRecord,
            newRecord,
            updatedFields,
            mandatoryFields

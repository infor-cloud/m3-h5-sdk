...
export class IonApiServiceCore extends CoreBase implements IIonApiService {
   ...
   private executeApi<T>(
      ...
      headers[IonApiConstants.platformHeaderName] =
         IonApiConstants.platformHeaderValue;
      ...
      this.httpService.execute(options).subscribe(
         ...
         (error: IHttpResponse) => {
            ...
            if (!isRetry && this.canRetry(options, error.status)) {
               this.getContext({ refresh: true }).subscribe(
                  (retryContext: IIonApiContext) => {
                     this.executeApi(
                        retryContext,
                        options,
                        source,
   ...
   private loadToken(isForceRefresh: boolean): void {
      ...
      const url =
         '/grid/rest/security/sessions/oauth?' +
         force +
         'rid=' +
         CoreUtil.random();
      ...
      this.httpService.execute(request).subscribe(
         (response: IHttpResponse) => {
            ...
            if (StringUtil.isNullOrEmpty(context.getUrl())) {
               ...
               this.formService.getEnvironmentContext().subscribe(
                  (envContext) => {
